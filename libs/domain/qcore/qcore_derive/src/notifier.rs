...
pub fn derive_notifier(input: TokenStream) -> TokenStream {
    ...
    let base_field = parse_notifier_attrib_to_specify_field(&class.attrs);
    ...
    let (field, field_name) = match (&class.data, &base_field) {
        (_, None) => panic!(
            "#[derive(Notifier)] must be used with #[notifier(transparent = ...)] to specify field to define notifier trait"
        ),
        (Data::Enum(_), _) => panic!("#[derive(Notifier)] can only be used on struct"),
        (Data::Union(_), _) => panic!("#[derive(Notifier)] can only be used on struct"),
            ...
            match &s.fields {
                Fields::Named(fs) => {
                    let Some(field) = fs.named.iter().find(|f| f.ident.as_ref() == Some(name)) else {
                        panic!("field named {name} is specified as base notifier but it is not found");
                    };
                    (field, quote!(#name))
                }
    ...
    where_clause
        .predicates
        .push(parse_quote!(#ty: #root ::datasrc::Notifier));
    ...
    quote!(
        impl #impl_generics #root ::datasrc::Notifier for #name #ty_generics #where_clause {
            ...
            #[inline]
            fn remove_listener(&mut self, id: &#root ::datasrc::NodeId) {
                self. #field_name .remove_listener(&id)
            }
        }
    )
    .into()
...
fn parse_notifier_attrib_to_specify_field(attribs: &[Attribute]) -> Option<Either<u16, Ident>> {
    let attribs: Vec<_> = attribs
        .iter()
        .filter_map(|attr| attr.meta.require_list().ok())
        .filter(|l| l.path.is_ident("notifier"))
        .filter_map(|l| {
    ...
    let value = match attribs.len() {
        ...
        _ => panic!("Multiple `notifier(transparent = ...)` attributes are not allowed"),
    };
    match value {
        syn::Lit::Int(n) => {
        ...
        _ => panic!("only integer or string is allowed as #[notifier(transparent = ...)]"),
    }
}
